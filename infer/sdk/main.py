...
def parse_args():
    ...
    parser.add_argument("--task_name", type=str, default="atis_intent", help="(atis_intent, mrda, swda, udc)")
...
def send_appsrc_data(args_opt, file_name, stream_name, stream_manager):
    ...
    Returns:
        bool: send data success or not
    """
    input_ids = os.path.realpath(os.path.join(args_opt.data_dir, "00_data", file_name))
    if not send_source_data(0, input_ids, stream_name, stream_manager):
        return False
    input_mask = os.path.realpath(os.path.join(args_opt.data_dir, "01_data", file_name))
    if not send_source_data(1, input_mask, stream_name, stream_manager):
        return False
    token_type_id = os.path.realpath(os.path.join(args_opt.data_dir, "02_data", file_name))
...
def run():
    ...
    for file_name in file_list:
        ...
        logit_id = post_process(args, file_name, infer_result)
