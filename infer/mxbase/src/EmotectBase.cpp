...
const uint32_t CLASS_NUM = 3;

APP_ERROR EmotectBase::Init(const InitParam &initParam) {
...
APP_ERROR EmotectBase::DeInit() {
    model_->DeInit();
    MxBase::DeviceManager::GetInstance()->DestroyDevices();
    return APP_ERR_OK;
}

APP_ERROR EmotectBase::ReadTensorFromFile(const std::string &file, uint32_t *data) {
    if (data == NULL) {
...
APP_ERROR EmotectBase::ReadInputTensor(const std::string &fileName, uint32_t index,
...
APP_ERROR EmotectBase::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EmotectBase::PostProcess(std::vector<MxBase::TensorBase> *outputs, std::vector<uint32_t> *argmax) {
...
APP_ERROR EmotectBase::CountPredictResult(const std::string &labelFile, const std::vector<uint32_t> &argmax) {
    uint32_t data[1] = {0};
...
APP_ERROR EmotectBase::WriteResult(const std::string &fileName, const std::vector<uint32_t> &argmax) {
...
APP_ERROR EmotectBase::Process(const std::string &inferPath, const std::string &fileName, bool eval) {
    std::vector<MxBase::TensorBase> inputs = {};
